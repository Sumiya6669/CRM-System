...
const ARCHIVABLE_TABLES = new Set(['inventory']);
...
const applyVisibility = (query, tableName, options = {}) => {
  let nextQuery = query;

  if (SOFT_DELETABLE_TABLES.has(tableName) && !options.includeDeleted) {
    nextQuery = nextQuery.eq('is_deleted', false);
  }

  if (ARCHIVABLE_TABLES.has(tableName) && !options.includeArchived) {
    nextQuery = nextQuery.eq('is_archived', false);
  }
...
export const createRepository = (entityName, tableName) => {
  ...
  return {
    ...
    async list(sort, limit = DEFAULT_LIMIT, options = {}) {
      const sortSpec = normalizeSort(sort);
      let query = applyVisibility(getClient().from(tableName).select('*'), tableName, options);
    ...
    async filter(filters = {}, sort, limit = DEFAULT_LIMIT, options = {}) {
      const sortSpec = normalizeSort(sort);
      let query = applyVisibility(applyFilters(getClient().from(tableName).select('*'), filters), tableName, options);
    ...
    async getById(id, options = {}) {
      return runSupabaseQuery(
        `${label}.getById`,
        applyVisibility(getClient().from(tableName).select('*'), tableName, options).eq('id', id).maybeSingle()
      );
    },
