...
export const authService = {
  ...
  async signIn(email, password) {
    ...
    void runSupabaseQuery(
      'functions.auth-events.login',
      getSupabaseClient().functions.invoke('auth-events', { body: { action: 'login' } })
    ).catch(() => {
      // Audit telemetry must not block a valid Supabase Auth session.
    });
    return result;
  },
