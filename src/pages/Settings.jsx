...
export default function Settings() {
  ...
  const saveBranchMutation = useMutation({
    mutationFn: (data) => editingBranch
      ? crm.entities.Branch.update(editingBranch.id, data)
      : crm.entities.Branch.create({ ...data, organization_id: activeOrganizationId, status: 'active' }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['branches'] });
      setShowAddBranch(false);
      setEditingBranch(null);
      setBranchForm({});
      toast.success(editingBranch ? 'Филиал обновлён' : 'Филиал добавлен');
    },
    onError: (error) => toast.error(error.message || 'Не удалось сохранить филиал'),
  });

  const deleteBranchMutation = useMutation({
    mutationFn: async ({ branch, reason }) => {
      await crm.entities.Branch.delete(branch.id);
      await crm.entities.ActivityLog.create({
        action_type: 'branch_deleted',
        description: `Филиал удалён: ${branch.name}${reason ? ` — ${reason}` : ''}`,
        entity_type: 'Branch',
        entity_id: branch.id,
        branch_id: branch.id,
    ...
    onError: (error) => {
      toast.error(error.message || 'Не удалось удалить филиал (проверьте права Owner)');
    },
  });
  ...
  return (
    <div>
          ...
          <TabsContent value="branches">
            <Card className="rounded-2xl">
              <CardHeader className="flex flex-row items-center justify-between gap-3">
                ...
                <Button size="sm" onClick={openNewBranch} className="gap-1 bg-primary hover:bg-primary/90">
      ...
      >
        <DialogContent>
          <DialogHeader><DialogTitle>{editingBranch ? 'Редактировать филиал' : 'Новый филиал'}</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!branchForm.name || !branchForm.city || !activeOrganizationId || saveBranchMutation.isPending}
              className="bg-primary hover:bg-primary/90"
            >
              {saveBranchMutation.isPending ? 'Сохранение...' : editingBranch ? 'Сохранить' : 'Создать'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <OwnerDeleteDialog
        ...
        title={branchToDelete?.name || 'Филиал'}
        details={[
          `Город: ${branchToDelete?.city || '—'}`,
          'Ученики/оплаты/продажи этого филиала останутся, но потеряют привязку к филиалу.',
        ]}
        isPending={deleteBranchMutation.isPending}
        onConfirm={(reason) => deleteBranchMutation.mutate({ branch: branchToDelete, reason })}
      />
