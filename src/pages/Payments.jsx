...
export default function Payments() {
  ...
  const unlockMutation = useMutation({
    mutationFn: (id) => documentService.unlock('payments', id),
    onSuccess: () => {
  ...
  const ownerDocumentMutation = useMutation({
    mutationFn: ({ id, action, reason }) => ownerActionsService.run(OWNER_RECORD_TYPES.PAYMENT, id, action, reason),
    onSuccess: (_, variables) => {
      qc.invalidateQueries({ queryKey: ['payments'] });
      setPaymentToDelete(null);
      toast.success({
        delete: 'Оплата перемещена в корзину',
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      if (editingPayment) {
        return crm.entities.Payment.update(editingPayment.id, data);
      }
    ...
    onSuccess: () => {
      ...
      setForm({});
      toast.success(editingPayment ? 'Оплата обновлена' : 'Оплата принята');
    },
    onError: (error) => toast.error(error.message),
  });
  ...
  const openPaymentEditor = (payment) => {
    ...
    <div>
      ...
      <div className="p-6 max-w-[1400px]">
        <PageHeader title="Управление оплатами" subtitle={`${filtered.length} записей · Итого: ${formatMoney(totalFiltered)}`}>
          <Button onClick={openNewPayment} className="gap-1.5 bg-primary hover:bg-primary/90">
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{editingPayment ? 'Редактировать оплату' : 'Принять оплату'}</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => createMutation.mutate(form)} disabled={!form.student_id || !form.amount || !form.payment_method || createMutation.isPending} className="bg-primary hover:bg-primary/90">
              {createMutation.isPending ? 'Сохранение...' : editingPayment ? 'Сохранить изменения' : 'Подтвердить оплату'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <OwnerDeleteDialog
        ...
        title={`Оплата: ${paymentToDelete?.student_name || '—'}`}
        details={[`Сумма: ${formatMoney(paymentToDelete?.amount || 0)}`]}
        isPending={ownerDocumentMutation.isPending}
        onConfirm={(reason) => ownerDocumentMutation.mutate({ id: paymentToDelete.id, action: 'delete', reason })}
      />
    </div>
  );
}
